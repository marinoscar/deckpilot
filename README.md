...
DeckPilot · conversational PowerPoint, powered by GitHub Copilot
  ...
  ❯ Start a new deck                    (s)
    Transform a deck                    (x)    ← restyle a deck's content 1:1 into a template's look
...
deckpilot transform --deck <pptx> --template <name> [<name>]   # restyle a deck's content 1:1 into a template's look
...
- ✅ **v1.3.2** — **Slash-menu highlight fix.** The `/` autocomplete dropdown's selected row used an invalid colour (`cyanBright` + `Bright` → `cyanBrightBright`), which ink silently drops — so the highlight never rendered. The selected `›` row now shows in the proper bright-cyan accent.
- ✅ **v1.3.3** — **Context-window visibility.** DeckPilot now surfaces how full the GitHub Copilot context window is, fed by the SDK's own `session.usage_info` / `assistant.usage` events (no token estimation of our own). The chat footer gains a **colour-coded `ctx` gauge** — green → yellow (≥60%) → red (≥85%) — showing the live fill percentage and `current/limit` tokens once the first turn lands. A new **`/context`** command prints a bordered report: a wide gauge with free-token headroom, a per-bucket **breakdown** (system prompt, tool definitions, conversation, message count), and **session totals** (API calls, input/output/reasoning tokens, cache read/write + cache-hit rate). The command registry, `/help`, and the tips footer pick it up automatically.
- ✅ **v1.3.4** — **Smart `@` picker — attach images & markdown inline.** Picking a file with `@` now does the right thing by type instead of only inserting a path: an **image** (`png`/`jpg`/`gif`/`webp`) stages as a **visual example** the model can see (like `/image`), and a **document** (`md`/`markdown`/`txt`/`pptx`/`docx`) has its **text extracted as context** (like `/doc`) — so `@notes.md` finally includes the markdown and `@logo.png` sends the image, both surfacing in the tray above the prompt. Any other file — or an `@` used inside a slash command such as `/template @brand.pptx` — still inserts the literal **path** as before. Each picker row is tagged with its kind (`[img]`/`[doc]`/`[pptx]`/…) and the routing is a pure, unit-tested `atActionFor()` helper.
- ✅ **v1.3.5** — **Transform is now a style transformation + a startup update check.** `deckpilot transform` was simplified to "**pick a deck, pick a template, done**": `deckpilot transform --deck deck.pptx --template <name>` (or **Transform a deck** in the TUI — step 1 pick the deck, step 2 pick the template) reproduces the deck's content **1:1** while adopting the chosen **template's** palette/fonts/brand. Unlike before, the agent **builds and saves automatically** — no manual "build" approval — then leaves the chat open for tweaks. The old two-deck `--original`/`--target` form is replaced by `--deck` + the normal template mechanism (a saved name *or* a one-shot `.pptx`), so a transform's style resumes from the project manifest like any other template. Also new: on launch DeckPilot does a **cached (once-a-day), non-blocking check** for a newer published version (the `version` on `main`) and shows a tidy `✨ vX.Y.Z is available` notice on the home screen — also surfaced by `deckpilot version` and as a soft `doctor` check. Opt out with `DECKPILOT_NO_UPDATE_CHECK=1`.
- ✅ **v1.3.6 (current)** — **Installer survives a locked `node_modules`.** Re-running the installer while a DeckPilot/Node instance was still open (or with antivirus mid-scan) made `npm ci` abort with a Windows `EPERM` / exit `-4048` as it tried to wipe `node_modules`, failing the whole update. Both installers now **fall back to `npm install`** when `npm ci` fails — `npm install` reconciles in place instead of demanding a clean wipe, so a stray lock no longer breaks the install — and the Windows installer prints an actionable hint (close `deckpilot`/`node`, exclude the folder from real-time scanning, or re-run with `-Reinstall`) instead of a raw runtime error.
