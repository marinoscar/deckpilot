...
> **Status:** v0.23 — **Transform a deck**: point DeckPilot at an **original** deck (the content) and a **target** deck (the brand/style), and it reproduces the original's content **1:1** — same slides, same wording, same speaker notes — in the target's palette, fonts, and chrome. Same content, new client. The agent reads the original's text *and* sees each source slide (a `study_original_slides` vision pass), proposes the slide-for-slide brief, pauses for your **build**, then auto-builds with the usual critique loop and stays open for chat. `deckpilot transform --original a.pptx --target b.pptx`, or pick **Transform a deck** in the menu.
>
