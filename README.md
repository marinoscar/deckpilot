...
> **Status:** v1.2 — **first stable release, now with diagnostics in the menu.** The full create → transform → improve workflow is in place, the install is pure-JS (no system packages), and the CLI/TUI surfaces are at parity. The first time you open the TUI, DeckPilot confirms GitHub Copilot is installed, signed in, and reachable before the menu — so you're not surprised by an auth wall mid-deck (shared with `deckpilot doctor`). The headline of the 1.0 release — **Improve a deck**: hand DeckPilot an existing deck and a brand **template**, and it quality-checks the deck and rebuilds a markedly better version. The agent reads the whole deck (full text *plus* a `study_source_slides` vision pass), writes a candid **improvement plan** saved to `IMPROVEMENT-PLAN.md` (overall assessment + per-slide recommendations), then proposes a rebuilt brief — sharper copy, stronger narrative, richer speaker notes, and a polished redesign in the template's palette/fonts/chrome. It pauses for your **build**, then auto-builds with the usual critique loop and stays open for chat. `deckpilot improve --source deck.pptx --template <name>` (optionally `--skill`), or pick **Improve a deck** in the menu.
>
> Also in v0.23 — **Transform a deck**: point DeckPilot at an **original** deck (the content) and a **target** deck (the brand/style) and it reproduces the original's content **1:1** — same slides, same wording, same speaker notes — in the target's palette, fonts, and chrome. "Same content, new client." `deckpilot transform --original a.pptx --target b.pptx`, or **Transform a deck** in the menu.
>
> Also in v0.22 — **Claude-Code-style chat UI**: the conversation renders like a modern agent CLI. Finalized turns are flushed once to your terminal's native scrollback (via ink's `<Static>`) so long sessions stay fast and scroll naturally, with `⏺` message bullets and `⎿` tool-result tree lines. The prompt is a rounded input box with a real movable caret (←/→, Ctrl+A/E) and multi-line editing (`\` + Enter, or Shift+Enter where supported), and **Esc** interrupts an in-flight generation. Menus and inputs share the same caret rendering.
>
...
- ✅ **v1.2.3 (current)** — **Paged deck picker for Transform & Improve.** The file picker that starts **Transform a deck** and **Improve a deck** now lists the workspace's `.pptx` files **newest-first, five at a time**. When there are more than five, a **Show more…** row pages through the rest (replacing the current five and cycling back to the first page after the last, with a `page X/Y` indicator); the **Type a path…** manual-entry row always sits at the end. The two flows now share one picker component instead of duplicating it.
