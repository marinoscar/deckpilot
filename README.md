...
- ✅ **v1.3.4 (current)** — **Smart `@` picker — attach images & markdown inline.** Picking a file with `@` now does the right thing by type instead of only inserting a path: an **image** (`png`/`jpg`/`gif`/`webp`) stages as a **visual example** the model can see (like `/image`), and a **document** (`md`/`markdown`/`txt`/`pptx`/`docx`) has its **text extracted as context** (like `/doc`) — so `@notes.md` finally includes the markdown and `@logo.png` sends the image, both surfacing in the tray above the prompt. Any other file — or an `@` used inside a slash command such as `/template @brand.pptx` — still inserts the literal **path** as before. Each picker row is tagged with its kind (`[img]`/`[doc]`/`[pptx]`/…) and the routing is a pure, unit-tested `atActionFor()` helper.
